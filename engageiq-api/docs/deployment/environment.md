...
| `APP_NAME` | no | `Mailvex-api` | Service name in logs |
...
| `REDIS_KEY_PREFIX` | no | `Mailvex:` | Key namespace |
...
| `NATS_NAME` | no | `Mailvex-api` | Client name |
...
| `JWT_ISSUER` | no | `Mailvex` | JWT `iss` claim |
| `JWT_AUDIENCE` | no | `Mailvex-api` | JWT `aud` claim |
