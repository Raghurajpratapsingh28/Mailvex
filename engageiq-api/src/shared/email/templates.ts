  ...
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#f4f4f5;padding:40px 0;">
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" width="560" style="background:#ffffff;border-radius:8px;padding:40px;max-width:560px;">
        <tr><td>
          <p style="margin:0 0 32px;font-size:22px;font-weight:700;color:#0f172a;">Mailvex</p>
          ${body}
          <hr style="border:none;border-top:1px solid #e2e8f0;margin:32px 0;" />
          <p style="margin:0;font-size:12px;color:#94a3b8;">You received this email because you have an account with Mailvex. If you didn't request this, you can safely ignore it.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function passwordReset(data: PasswordResetData): TemplateResult {
  const html = layout(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.6;">We received a request to reset your Mailvex password. Click the button below. This link expires in ${data.expiresInMinutes} minutes.</p>
  ...
  const text = `Reset your password\n\nWe received a request to reset your Mailvex password. Click the link below (expires in ${data.expiresInMinutes} minutes):\n\n${data.resetUrl}\n\nIf you didn't request a password reset, you can safely ignore this email.`;
...
function invite(data: InviteData): TemplateResult {
  const html = layout(`
    <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#0f172a;">You've been invited to Mailvex</h1>
...
function domainVerificationReminder(data: DomainReminderData): TemplateResult {
  const url = data.dashboardUrl ?? 'https://app.Mailvex.io/domains';
  const html = layout(`
